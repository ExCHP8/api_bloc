...
/// The base state class used in [WriteRequest].
///
/// The [WriteStates] class extends [BlocStates] to provide a generic state
/// that includes [message], and a nullable [data] object. This limited to five states in total.
/// The five states are [WriteIdleState], [WriteLoadingState], [WriteSuccessState], [WriteFailedState], and [WriteErrorState].
...
/// Represents the idle state when no request is in progress in [WriteRequest].
class WriteIdleState<T extends dynamic> extends WriteStates<T> {
  /// Create a [WriteIdleState] with an optional [message] and [data].
  const WriteIdleState({super.message, super.data});
}

/// Represents the loading state when a request is in progress in [WriteRequest].
class WriteLoadingState<T extends dynamic> extends WriteStates<T> {
...
/// [WriteRequest].
class WriteErrorState<T extends dynamic> extends WriteStates<T> {
  /// Represents the error state when an error occurs during request submission in
  /// [WriteRequest].
